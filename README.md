...
dotman 是一个自用的 dotfiles 管理脚本，采用 **Git 仓库保存真实文件 + 本机软链引用** 的模型。它很轻量，只依赖 `git`。
...
- 通过 INI 文件记录每个设备上应该建立哪些软链。
- 修改映射表或新增文件后会**自动提交**到本地仓库，并尝试推送；推送失败时仅给出警告，不会中断操作。
...
│   ├── config.conf                 # 仓库地址（repo_url）
│   ├── links.common.conf           # 所有设备共用的软链
│   ├── links.<hostname>.conf      # 当前设备特有的软链
│   └── .zshrc / .config/nvim/ ...  # 真实文件

~/.config/dotman/config.conf        # 本地配置：仓库路径 + 当前设备名
```
...
| `DOTMAN_REPO` | `~/.dotman/dotfiles` | dotfiles Git 仓库路径（显式设置时优先级最高） |

### 本地配置 `~/.config/dotman/config.conf`

```ini
repo_path = "/Users/you/.dotman/dotfiles"
hostname = "macbook-pro"
```

### 仓库配置 `~/.dotman/dotfiles/config.conf`
...
**合并规则**：`links.common.conf` 先加载，设备特定文件后加载并覆盖同名目标。设备特定链接优先级更高。
...
交互式启动时，如果仓库配置了远程地址，会自动先执行一次 `git pull`。
...
操作菜单：
  ...
  5. 移除并恢复文件             dotman remove
  6. 检查链接状态               dotman status
  7. 一键修复链接               dotman fix
  8. 综合诊断                   dotman doctor
  0. 退出
...
1. 把源文件/目录移动到仓库中对应位置。
2. 在原位置创建指向仓库的软链。
3. 把映射写入 `links.<hostname>.conf`。
4. 自动 `git commit`，并尝试 `git push`（推送失败仅警告，不中断）。

**路径填写规则**：
- 绝对路径：`/Users/you/.zshrc`
- `~` 路径：`~/.zshrc`
- 相对路径：`bin`、`.zshrc`、`.config/nvim` 都会自动解析为 `~` 下路径

**冲突处理**：如果仓库中已存在同名文件，`add` 会提示三选一：
1. 用本地版本覆盖仓库版本（默认）
...
`link` / `unlink` 主要在 CLI/脚本场景使用，交互式菜单未暴露这两个操作。
...
- 设备名优先读取本地配置 `~/.config/dotman/config.conf` 中的 `hostname`；未配置时使用 `hostname -s` 的值。
- 通用配置放在 `links.common.conf`。
- 设备特定配置放在 `links.<hostname>.conf`。
...
# links.macbook-pro.conf
[links]
.hammerspoon = ~/.hammerspoon

# links.workstation.conf
[links]
.config/i3 = ~/.config/i3
```
...
如果该文件映射在 `links.common.conf` 中，B 设备会自动建立软链。如果只在 A 设备的 `links.<hostname>.conf` 中，B 设备不会自动建立，需要手动添加映射。

### Q: 为什么修改映射表后要重新运行 `fix`？

`links.*.conf` 只是声明了"应该存在哪些软链"。`status` 用于检查，`fix` 用于应用这些声明到本机文件系统。

### Q: 我不想每次都 push，可以只本地提交吗？

当前版本所有会修改仓库文件的操作都会尝试 push，但 push 失败不会中断流程，只会给出警告。如果你需要更细粒度的控制，可以直接进入仓库目录手动使用 `git` 命令。

---

## 设计取舍

- **INI 而非 TOML**：更契合 Shell 原生处理能力，无需额外解析器，只使用扁平 key/value 即可满足需求。
- **软链而非复制**：保持单份真实来源，本机编辑即仓库编辑。
- **设备特定文件覆盖 common**：简单明确，避免复杂的条件表达式。
- **自动提交、尝试推送**：自用脚本，减少手动步骤；推送失败不中断，避免网络问题影响本地操作。
